...
def test_audit_log_save_view(client, request_response_data, audit_url):
    email = request_response_data['audit_log'][0]['payload']['email']
    ...
    request_bink_message_uid = request_response_data['audit_log'][0]['bink_message_uid']

    assert stored_member.last_name == request_response_data['audit_log'][0]['payload']['last_name']
    assert len(request) == 1
    assert str(request.first().bink_message_uid) == request_bink_message_uid

    response = request.first().responses.all()
    response_status_code = request_response_data['audit_log'][1]['status_code']
...
def test_fail_without_token(client, request_response_data, audit_url):
    response = client.post(path=audit_url, data=request_response_data)
